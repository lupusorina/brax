...
def train(environment: envs.Env,
          ...
          tau: float = 0.005,
          min_replay_size: int = 0,
          max_replay_size: Optional[int] = 2_000_000,
          grad_updates_per_step: int = 1,
          deterministic_eval: bool = False,
          wrap_env: bool = True,
          wrap_env_fn: Optional[Callable[[Any], Any]] = None,
          ...
          checkpoint_logdir: Optional[str] = None):
  """
  Trains a DDPG agent and optionally saves checkpoints at halfway and final.
  """
  ...
  def sgd_step(
    ...
    critic_loss_value, q_params, q_optimizer_state = critic_update(
        training_state.q_params,
        training_state.policy_params,
        training_state.normalizer_params,
        training_state.target_q_params,
    ...
    (actor_loss_value, actor_info), policy_params, policy_optimizer_state = actor_update(
        training_state.policy_params,
        training_state.normalizer_params,
        training_state.target_q_params,
        transitions,
  ...
  def training_step(
    ...
    buffer_state, transitions = replay_buffer.sample(buffer_state)
    # reshape transitions for multiple sgd steps
  ...
  # for measuring total training time
  def training_epoch_with_timing(
      training_state: TrainingState, env_state: envs.State,
      buffer_state: ReplayBufferState, key: PRNGKey
  ) -> Tuple[TrainingState, envs.State, ReplayBufferState, Metrics]:
  ...
  replay_size = jnp.sum(
      jax.vmap(replay_buffer.size)(buffer_state)) * jax.process_count()
  logging.info('replay size after prefill %s', replay_size)
  assert replay_size >= min_replay_size
  training_walltime = time.time() - t

  # Track when we have saved the half checkpoint
  half_ckpt_saved = False
  ...
  for _ in range(num_evals_after_init):
    ...
    (training_state, env_state, buffer_state, training_metrics
     ) = training_epoch_with_timing(training_state, env_state, buffer_state,
                                    epoch_keys)

    current_step = int(_unpmap(training_state.env_steps))

    # Save halfway checkpoint if desired
    if checkpoint_logdir and (not half_ckpt_saved) and (current_step >= half_timesteps):
      params_half = _unpmap(
          (training_state.normalizer_params, training_state.policy_params))
      path_half = f'{checkpoint_logdir}_ddpg_halfway.pkl'
      model.save_params(path_half, params_half)
      half_ckpt_saved = True

    # For TensorBoard
    if tensorboard_flag:
      with tf.name_scope('Metric Info'):
        tf.summary.scalar(
            'actor_loss', data=np.array(training_metrics['training/actor_loss']),
            step=current_step)
        tf.summary.scalar(
            'critic_loss', data=np.array(training_metrics['training/critic_loss']),
            step=current_step)
        tf.summary.scalar(
            'raw_action_mean',
            data=np.array(training_metrics['training/raw_action_mean']),
            step=current_step)
        tf.summary.scalar(
            'raw_action_std',
            data=np.array(training_metrics['training/raw_action_std']),
            step=current_step)
        tf.summary.scalar(
            'sampled_action_mean',
            data=np.array(training_metrics['training/sampled_action_mean']),
            step=current_step)
        tf.summary.scalar(
            'sampled_action_std',
            data=np.array(training_metrics['training/sampled_action_std']),
            step=current_step)

    # Final or intermediate checkpoint + eval
    if process_id == 0:
      if checkpoint_logdir:
        # Always save current policy (could be at multiple intervals)
        params = _unpmap(
            (training_state.normalizer_params, training_state.policy_params))
        path = f'{checkpoint_logdir}_ddpg_{current_step}.pkl'
        model.save_params(path, params)
  ...
  # If there was no mistake the training_state should still be identical on all
  # devices.
  pmap.assert_is_replicated(training_state)
  logging.info('total steps: %s', total_steps)
  pmap.synchronize_hosts()
